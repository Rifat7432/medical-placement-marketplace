...
const verifyEmailToDB = async (payload: IVerifyEmail) => {
     ...
     if (isExistUser.oneTimeCode !== oneTimeCode) {
          throw new AppError(StatusCodes.BAD_REQUEST, 'You provided wrong otp');
     }

     const date = new Date();
     if (date > (isExistUser.OTPExpireAt as Date)) {
     ...
     let user;
     if (!isExistUser.verified) {
          await User.findOneAndUpdate({ _id: isExistUser._id }, { verified: true, oneTimeCode: null, OTPExpireAt: null });
          //create token
          ...
          await User.findOneAndUpdate({ _id: isExistUser._id }, { isResetPassword: true, oneTimeCode: null, OTPExpireAt: null });
...
const resetPasswordToDB = async (token: string, payload: IAuthResetPassword) => {
     ...
     const updateData = { password: hashPassword, isResetPassword: false };
...
const resetPasswordByUrl = async (token: string, payload: IAuthResetPassword) => {
     ...
     // Update Password
     await User.findByIdAndUpdate(id, { password: hashPassword, isResetPassword: false }, { new: true, runValidators: true });
